...
export default function ExportModal({ isOpen, onClose, content, title, tier }: ExportModalProps) {
  ...
  // Update format when tier changes or modal opens
  useEffect(() => {
    if (isOpen) {
      setFormat(tier === 'starter' ? 'pdf' : 'markdown');
    }
  }, [isOpen, tier]);
  ...
  const generatePDF = (title: string, content: string) => {
    ...
    const watermark = tier === 'starter' ? "\n\n---\nGenerated by NicheRadar (Starter Plan)" : "";
    const fullContent = content + watermark;
    
    // Split text to fit page width
    const splitText = doc.splitTextToSize(fullContent, 170);
          ...
          <div className="space-y-4 mb-8">
            <button
              ...
              className={`w-full flex items-center gap-3 p-4 rounded-xl border transition-all ${
                tier === 'starter' ? "opacity-50 cursor-not-allowed bg-slate-800/20 border-slate-800 text-slate-500" :
            ...
            >
              ...
              <div className="text-left">
                <div className="font-bold flex items-center gap-2">
                  Markdown (.md)
                  {tier === 'starter' && <span className="text-[10px] bg-slate-700 px-2 py-0.5 rounded-full text-slate-300">Pro+</span>}
                </div>
            ...
            >
              ...
              <div className="text-left">
                <div className="font-bold flex items-center gap-2">
                  PDF Document 
                  {tier === 'starter' && <span className="text-[10px] bg-slate-700 px-2 py-0.5 rounded-full text-slate-300">Watermarked</span>}
                </div>
